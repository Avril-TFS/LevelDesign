...
public class DisksController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (TowerOfHanoiManager.towerManager.CanPlay && !_isMoving)
        {
            /* if (TowerOfHanoiManager.towerManager.IsAutoModeOn)
             {
                 if (AutoSolver.GetAutoMovesCount() > 0 && Time.time >= _waitTime)
                 {
                     ...
                     TowerOfHanoiManager.towerManager.CurrentMoves++;
                     TowerOfHanoiManager.towerManager.RemainingMoves--;

                     //TODO: Change UpdateCurrentMoves and UpdateRemainingMoves with Events.
                     // Update UI
                     //  UIManager.uIManager.UpdateCurrentMoves(towerManager.towerManager.CurrentMoves);
                     //  UIManager.uIManager.UpdateRemainingMoves(towerManager.towerManager.RemainingMoves);

                     // Set up the moving disk components
                     _gameObject = TowerOfHanoiManager.towerManager.MoveTopDisk(_moveFromKey, _moveToKey);
                     ...
                     if (TowerOfHanoiManager.towerManager.HasLevelFinished())
                     {
                         // UIManager.uIManager.ShowLevelCompletePanel();
                         //  UIManager.uIManager.HidePlaymodeUI();
                         TowerOfHanoiManager.towerManager.CanPlay = false;
                     }
                 }*/
        }
        else
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (Physics.Raycast(ray, out hit))
                {
                    ...
                    {
                        if (TowerOfHanoiManager.towerManager.IsTowerSelectedValid(hit, true, _moveFromKey))
                        {
                            _moveFromKey = TowerOfHanoiManager.towerManager.GetTowerKey(hit.collider.tag);

                            GameObject disk = TowerOfHanoiManager.towerManager.PeekTower(_moveFromKey);
                    ...
                    {
                        if (TowerOfHanoiManager.towerManager.IsTowerSelectedValid(hit, false, _moveFromKey))
                        {
                            _moveToKey = TowerOfHanoiManager.towerManager.GetTowerKey(hit.collider.tag);

                            _canSaveMoveToKey = false;

                            if (TowerOfHanoiManager.towerManager.IsMoveSelectedValid(_moveFromKey, _moveToKey))
                            {
