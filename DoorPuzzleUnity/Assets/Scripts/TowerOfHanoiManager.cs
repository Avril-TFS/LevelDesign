...
{
    #region Fields
    public static TowerOfHanoiManager towerManager;
    ...
    private void Awake()
    {
        ...
        {
            towerManager = GetComponent<TowerOfHanoiManager>();
        }
    ...
    public bool IsMoveSelectedValid(char moveFromKey, char moveToKey)
    {
        ...
        if (_towers[moveToKey].Count > 0)
        {
            ...
            int moveFromRank = moveFromTopDisc.GetComponent<Discs>().rank;

            GameObject moveToTopDisc = (GameObject)_towers[moveToKey].Peek();
            int moveToRank = moveToTopDisc.GetComponent<Discs>().rank;
